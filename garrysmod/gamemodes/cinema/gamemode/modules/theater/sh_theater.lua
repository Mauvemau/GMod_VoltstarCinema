...
if SERVER then
	...
	function THEATER:Reset()
		...
		if self:IsPrivate() then
			...
			self._Owner = nil
			WipeBlacklist(self.Id)
		end

	end
	
	function THEATER:HardReset()
	...
	*/
	...
	function THEATER:NextVideo()
		...
		if SERVER then
			if(self._Video._VideoData != self._LastPlayed)then
				local thID = self:GetLocation()
				hook.Call("chsh_cinema_video_rewards", nil, thID, self._StartedPlaying, self._Video._VideoData)
			end
		end
	...
	*/
	...
	function THEATER:SetBlacklist(tbl)
		if(!self:IsPrivate()) then return end
		self.Blacklist = tbl
	
	end
	
	function THEATER:GetBlacklist()
